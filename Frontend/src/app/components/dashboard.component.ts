  ...
  template: `
    <div class="iot-game-container">
      ...
      <div class="game-world">
        ...
        <div class="robot" 
          ...
          <div class="alert-effect" *ngIf="alertaActiva">
            ...
            <div class="danger-particles">
              <span *ngFor="let i of [1,2,3,4,5,6]" class="danger-particle">⚠️</span>
            </div>
          </div>
        ...
        <!-- Ambientes -->
        <div *ngFor="let ambiente of ambientes" 
             class="ambiente"
          ...
          <div class="ambiente-status" *ngIf="ambienteActual?.id === ambiente.id">
        ...
        <svg class="robot-path" width="100%" height="100%">
          <path *ngIf="caminoActual.length > 0" 
      ...
      <div class="mission-status" *ngIf="misionActiva">
        <div class="mission-info">
          ...
          <p>🎯 Objetivo: {{ambienteObjetivo?.nombre || 'Calculando ruta...'}}</p>
      ...
      <div class="temperature-alerts" *ngIf="alertasRecientes.length > 0">
        <div *ngFor="let alerta of alertasRecientes" 
             ...
             [@slideIn]>
          🚨 <strong>Alerta:</strong> {{alerta.ambiente}} registró {{alerta.temperatura}}°C
      ...
      <div class="stats-panel">
        ...
        <div class="stat-card environments">
          <div class="stat-icon">🏠</div>
          <div class="stat-content">
            <div class="stat-value">{{ambientesVisitados.length}}/{{ambientes.length}}</div>
      ...
      <div class="chart-section">
        <div class="chart-header">
          ...
          <div class="chart-controls">
            <button class="chart-btn" (click)="cambiarVistaGrafica('linea')" 
                    [class.active]="vistaGrafica === 'linea'">📊 Línea</button>
            <button class="chart-btn" (click)="cambiarVistaGrafica('barras')" 
                    [class.active]="vistaGrafica === 'barras'">📈 Barras</button>
          </div>
        </div>
        ...
        <div class="chart-legend">
          <div class="legend-item" *ngFor="let ambiente of ambientes">
            <span class="legend-color" [style.background-color]="ambiente.color"></span>
      ...
      <footer class="game-footer">
        <div class="connection-status" [class.connected]="!error" [class.disconnected]="error">
...
export class DashboardComponent implements OnInit, OnDestroy {
  ...
  comenzarCicloExploracion(): void {
    ...
    const ambientesDisponibles = this.ambientes.filter(a => 
      !this.ambientesVisitados.includes(a.id) || this.ambientesVisitados.length === this.ambientes.length
    );
    ...
    this.ambienteObjetivo = ambientesDisponibles[Math.floor(Math.random() * ambientesDisponibles.length)];
    this.mostrarMensajeRobot(`🎯 Dirigiéndome a ${this.ambienteObjetivo.nombre}`);
  ...
  moverRobotA(ambiente: Ambiente): void {
    ...
    this.robotEstado = 'walking';
    this.caminoActual = this.calcularCamino(this.robotPosicion, ambiente.posicion);
  ...
  simularLectura(): void {
    ...
    this.mostrarMensajeRobot(`✅ Lectura completada: ${temperaturaRedondeada}°C`);
  ...
  activarAlerta(): void {
    ...
    this.mostrarMensajeRobot(`🚨 ¡Alerta! Temperatura alta: ${this.ultimaTemperatura}°C`);
  ...
  calcularCamino(inicio: { x: number; y: number }, fin: { x: number; y: number }): { x: number; y: number }[] {
    return [inicio, fin];
  }
  ...
  toggleModoDemo(): void {
    ...
    this.mostrarMensajeRobot(this.modoDemo ? 
  ...
  toggleEfectosSonido(): void {
    ...
    this.mostrarMensajeRobot(this.efectosSonido ? 
